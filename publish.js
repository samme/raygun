...
// var scopeToPunc = helper.scopeToPunc; // unused
...
function getSignatureAttributes (item) {
  ...
  if (item.optional) {
    attributes.push('<abbr aria-label="optional" title="optional">?</abbr>');
  }

  if (item.nullable === true) {
    attributes.push('<abbr aria-label="nullable" title="nullable">∅</abbr>');
  } else if (item.nullable === false) {
    attributes.push('<abbr aria-label="nonnull" title="nonnull">!</abbr>');
  }
...
function buildMemberNav (items, itemHeading, itemsSeen, linktoFn) {
  ...
  if (items && items.length) {
    ...
    items.forEach(function (item) {
      ...
      // var members = find({kind: 'member', memberof: item.longname}); // unused
